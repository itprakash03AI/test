...
def authenticate_user(username: str, password: str) -> Optional[dict]:
    ...
    **Does NOT check the super_admin table.** This function is called on
    ...
    # Always use SIMPLE auth with UPN format (user@domain).
    # NTLM is deprecated by Microsoft and uses MD4 hashing which is rejected
    # by OpenSSL's FIPS policy on OpenShift — causing a hard crash.
    # SIMPLE + UPN works on both ldap:// and ldaps://.
    ...
    logger.info(
        ...
        "normalized_url=%r → use_ssl=%s → auth_method=SIMPLE (UPN)",
        username, server_url, _auth.ad_use_ssl, domain,
        _server_url_norm, _use_ssl,
    )
